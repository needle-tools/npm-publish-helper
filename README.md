...
jobs:
  run-release-script:
    ...
    steps:
      ...
      - name: Publish to npm
        id: publish
        run: npx --yes needle-publish-helper@stable publish "." --prepare-package --webhook "${{ secrets.DISCORD_WEBHOOK }}" --access-token "${{ secrets.NPM_TOKEN }}" --tag "${{github.ref_name}}" --version+tag --version+hash --create-tag release/
```
