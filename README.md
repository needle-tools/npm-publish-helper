...
| `--tag <tag>`           | string  | Sets the NPM dist-tag to apply. Can be passed multiple times (e.g. `--tag version-2 --tag latest`); the first tag is applied during `npm publish`, the rest via `npm dist-tag add` (works with `--oidc`). |         |
...
| `--provenance`        | boolean | Generate provenance attestations (auto-enabled with `--oidc` for public repos). Set to `false` to disable. Only works with public repositories.                                   | auto    |
